...
public class SplitArray3 {
  ...
  public int splitArray(int[] nums, int m) {
    ...
    i = 0;
    int j = i;
    double avg = (double)ps[n - 1] / m;
    //System.out.println("ps: " + Arrays.toString(ps) + ", avg: " + avg);
    //System.out.println("nums:" + Arrays.toString(nums));

    for (; i < n; i++) {
      int s = j == 0 ? ps[i] : ps[i] - ps[j - 1];

      if (s > avg) {
        double g1 = Math.abs(s - avg);
        double g2 = Math.abs(s - nums[i] - avg);
        if (g2 <= g1) {
          subArrays.add(new SubArray(j, i, s - nums[i]));
        } else {
          subArrays.add(new SubArray(j, i+1, s));
	  i++;
        }
        j = i;
      }
    ...
    System.out.println("subArray.size(): " + subArrays.size() );
    return max.sum;
  }
  ...
  public static void main(String[] args) {
    ...
    int r = new SplitArray3().splitArray(A, m);
